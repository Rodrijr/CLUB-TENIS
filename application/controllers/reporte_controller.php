...
class Reporte_controller extends CI_Controller 
{
  ...
  //vista del visitante
    ...
    public function generar_pdf_alumnos()
    {
        $this->load->library('pdf');
        $pdf = new Pdf('P', 'mm', 'A4', true, 'UTF-8', false);
        $pdf->SetCreator(PDF_CREATOR);
        ...
        #$html .= "td{background-color: #AAC7E3; color: #fff}";
        $html .= "</style>";
        $html .= "<h2>Lista de Alumnos: </h2>";
        $html .= "<table width='100%'>";
        $html .= "<tr><th>Ci</th><th>Nombre</th><th>Apellidos</th><th>Telefono</th><th>Email</th></tr>";
        
        //provincias es la respuesta de la función getProvinciasSeleccionadas($provincia) del modelo
        foreach ($persona as $alumnos) 
        {
            $html .= "<tr><td class='id'>" .$alumnos['ci_persona']. "</td><td class='localidad'>" .$alumnos['nombre_persona']."</td><td class='id'>".$alumnos['apellido_persona']."</td><td class='localidad'>" .$alumnos['telefono']. "</td><td class='localidad'>" .$alumnos['email']. "</td></tr>";
        }
        $html .= "</table>";
    ...
    public function generar_pdf_sub_grupo_entrenador()
    {
        ...
        $lista_sub_grupos = $this->input->post('lista_grupos');
        print_r($lista_sub_grupos);
        ...
        foreach($lista_sub_grupos as $sub_grupo_entrenador){

            $html .= "<h2>".$sub_grupo_entrenador['sub_grupo']['nombre_subgrupo']." - ".$sub_grupo_entrenador['grupo']['nombre_grupo']."</h2>";
            $html .= "<h4> <strong>Entrenador(es): </strong>".$sub_grupo_entrenador['entrenador']."</h4>";
            $html .= "<h4> <strong>Horario: </strong>".$sub_grupo_entrenador['sub_grupo']['horario']."</h4>";
            $html .= "<h4> ".$sub_grupo_entrenador['sub_grupo']['descripcion']."</h4>";
            $html .= "<legend><center>Alumnos</center></legend>>";
            $html .= '<table class="table table-striped table-hover" width="100%">';
            $html .= "<tr><th>#</th><th>Nombre</th><th>Apellidos</th></tr>";
            $cont = 1;
            foreach($sub_grupo_entrenador['lista_alumnos'] as $alumno){
              $html .= "<tr>";
                $html .= "<td>".$cont."</td>";
                $html .= "<td>".$alumno['nombre_persona']."</td>";
                $html .= "<td>".$alumno['apellido_persona']."</td>";
              $html .= "</tr>";
              $cont++;
            }
            $html .= "</table>";
